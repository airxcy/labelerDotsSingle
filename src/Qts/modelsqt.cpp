...
{
    if(event->button()==Qt::LeftButton&&trkscene->isEditing)
    {
        selected=true;
...
void DragDots::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    painter->setPen(linepen);
    painter->drawRect(boundingRect());
    painter->setPen(QColor(rgb[0],rgb[1],rgb[2],200-ismovable*100));
...
Dot::Dot(int x, int y, QGraphicsItem *parent):QGraphicsItem(parent)
{
    coord[0]=x,coord[1]=y;
    pid=-1;
    range=20;
...
void Dot::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    painter->setPen(linepen);
    painter->drawRect(boundingRect());
    painter->setPen(QColor(rgb[0],rgb[1],rgb[2],50));
    painter->drawLine(coord[0]-range,coord[1],coord[0]+range,coord[1]);
    painter->drawLine(coord[0],coord[1]-range,coord[0],coord[1]+range);
    //painter->setPen(QColor(rgb[0],rgb[1],rgb[2],255));
    //painter->setFont(txtfont);
    //painter->drawText(QRectF(coord[0]+range,coord[1],range,range), Qt::AlignCenter|Qt::AlignCenter,txt);
}
void Dot::setCoord(int x,int y)
{
    prepareGeometryChange();
    coord[0]=x,coord[1]=y;
}
void Dot::setClr(unsigned char r,unsigned char g,unsigned char b)
{
    rgb[0]=r,rgb[1]=g,rgb[2]=b;
    rectBrush.setColor(QColor(r,g,b,10));
    rectBrush.setStyle(Qt::SolidPattern);
